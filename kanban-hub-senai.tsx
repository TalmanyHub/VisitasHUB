...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        /* migração: fit antigo era string; responsáveis pode não existir */
        const migrated = raw.map((l) => ({
          ...l,
          fit: Array.isArray(l.fit) ? l.fit : (l.fit ? [l.fit] : []),
          responsaveis: Array.isArray(l.responsaveis) ? l.responsaveis : [],
        }));
        setLeads(migrated);
      } catch { setLeads(SEED); }
      setLoaded(true);
  ...
  const persist = useCallback(async (data) => {
    try { const r = await window.storage.set(STORE_KEY, JSON.stringify(data)); setSaveErr(!r); }
  ...
  const saveLead = (lead) => {
    ...
    update(exists ? leads.map((l) => (l.id === lead.id ? lead : l)) : [...leads, lead]);
    setEditing(null);
  };
  const delLead = (id) => update(leads.filter((l) => l.id !== id));
  ...
  if (!loaded)
    return <div style={{ padding: 40, fontFamily: "system-ui", color: C.ink3 }}>Carregando…</div>;
    ...
    <>
      ...
      {saveErr && (
        <div style={{ margin: "8px 26px 0", padding: "8px 12px", background: C.redBg,
          ...
          ⚠ Não foi possível salvar. Verifique a conexão — alterações podem se perder.
        </div>
      )}
